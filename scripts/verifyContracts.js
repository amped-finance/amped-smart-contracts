...
// --- Configuration ---
// Get deployment log file from command line argument or environment variable
const LOG_FILE = process.argv[2] || process.env.DEPLOYMENT_LOG || `./deployment-log-${hre.network.name}.json`;
...
async function main() {
    ...
    if (!fs.existsSync(LOG_FILE)) {
        console.error(`Deployment log file not found: ${LOG_FILE}`);
        console.log(`Usage: npx hardhat run scripts/verifyContracts.js --network <network> [deployment-log-file.json]`);
        console.log(`Or set DEPLOYMENT_LOG environment variable`);
        process.exit(1);
    }
