...
async function getArbValues() {
  ...
  const esAmp = await contractAt("Token", "0xf42Ae1D54fd613C9bb14810b0588FaAa09a426cA")
  const nativeTokenPrice = ethPrice
  const data = arbitrumData

  return { batchSender, esAmp, nativeTokenPrice, data }
}

async function getAvaxValues() {
  const batchSender = await contractAt("BatchSender", "0xF0f929162751DD723fBa5b86A9B3C88Dc1D4957b")
  const esAmp = await contractAt("Token", "0xFf1489227BbAAC61a9209A08929E4c2a526DdD17")
...
async function main() {
  ...
  const { batchSender, esAmp, nativeTokenPrice, data } = await getValues()
  ...
  let totalEsAmpAmount = bigNumberify(0)
  ...
  for (let i = 0; i < affiliatesData.length; i++) {
    const { account, rebateUsd, esampRewardsUsd } = affiliatesData[i]
    ...
    if (esampRewardsUsd) {
      const esAmpAmount = bigNumberify(esampRewardsUsd).mul(expandDecimals(1, 18)).div(expandDecimals(ampPrice, 30))
      esAmpAccounts.push(account)
      esAmpAmounts.push(esAmpAmount)
      totalEsAmpAmount = totalEsAmpAmount.add(esAmpAmount)
    }
  }
  ...
  console.log(`total esAmp`, ethers.utils.formatUnits(totalEsAmpAmount, 18))
  ...
  if (shouldSendTxn) {
    ...
    await sendTxn(esAmp.approve(batchSender.address, totalEsAmpAmount), "esAmp.approve")

    await processBatch([esAmpAccounts, esAmpAmounts], batchSize, async (currentBatch) => {
      printBatch(currentBatch)

      const accounts = currentBatch.map((item) => item[0])
      const amounts = currentBatch.map((item) => item[1])

      await sendTxn(batchSender.sendAndEmit(esAmp.address, accounts, amounts, affiliateRewardsTypeId), "batchSender.sendAndEmit(nativeToken, esAmp affiliate rewards)")
    })
  }
}
