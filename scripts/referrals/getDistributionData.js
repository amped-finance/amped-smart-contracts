...
const ARBITRUM_SUBGRAPH_ENDPOINT = 'https://api.thegraph.com/subgraphs/name/amp-io/amp-arbitrum-referrals'
const AVALANCHE_SUBGRAPH_ENDPOINT = 'https://api.thegraph.com/subgraphs/name/amp-io/amp-avalanche-referrals'
const CRONOS_SUBGRAPH_ENDPOINT = 'https://graph-node.ampespade.xyz/subgraphs/name/graphprotocol/referrals'

const BigNumber = ethers.BigNumber
const { formatUnits, parseUnits } = ethers.utils
const SHARE_DIVISOR = BigNumber.from("1000000000") // 1e9
const BONUS_TIER = 2 // for EsAMP distributions
const USD_DECIMALS = 30
const AMP_DECIMALS = 18
...
function getSubgraphEndpoint(network) {
  return {
    ...
    cronos: CRONOS_SUBGRAPH_ENDPOINT
  }[network]
}
...
async function queryDistributionData(network, fromTimestamp, toTimestamp, account, ampPrice, esampRewards) {
  if (ampPrice) {
    ampPrice = parseUnits(ampPrice, USD_DECIMALS)
  }
  if (esampRewards) {
    esampRewards = parseUnits(esampRewards, AMP_DECIMALS)
  }
  ...
  if (ampPrice && esampRewards) {
    const esampRewardsUsdLimit = esampRewards.mul(ampPrice).div(expandDecimals(1, AMP_DECIMALS))
    let esampRewardsUsdTotal = BigNumber.from(0)
    Object.values(referrersRebatesData).forEach(data => {
      if (data.tierId !== BONUS_TIER) {
        return
      }
      data.esampRewardsUsd = data.volume.div(1000).div(20) // 0.1% margin fee, 0.05% of fee is EsAMP bonus rewards
      data.esampRewards = data.esampRewardsUsd
        .mul(expandDecimals(1, USD_DECIMALS))
        .div(ampPrice)
        .div(expandDecimals(1, 12))
      esampRewardsUsdTotal = esampRewardsUsdTotal.add(data.esampRewardsUsd)
    })

    if (esampRewardsUsdTotal.gt(esampRewardsUsdLimit)) {
      const denominator = esampRewardsUsdTotal.mul(USD_DECIMALS).div(esampRewardsUsdLimit)
      Object.values(referrersRebatesData).forEach(data => {
        data.esampRewardsUsd = data.esampRewardsUsd.mul(USD_DECIMALS).div(denominator)
        data.esampRewards = data.esampRewardsUsd
          .mul(expandDecimals(1, USD_DECIMALS))
          .div(ampPrice)
          .div(expandDecimals(1, 12))
  ...
  for (const data of Object.values(referrersRebatesData)) {
    ...
    consoleData.push({
      ...
      tierId: data.tierId,
      "esampRewards, $": data.esampRewardsUsd ? formatUnits(data.esampRewardsUsd, USD_DECIMALS) : null,
      esampRewards: data.esampRewards ? formatUnits(data.esampRewards, AMP_DECIMALS) : null,
    })
    output.referrers.push({
      ...
      tierId: data.tierId,
      esampRewards: data.esampRewards ? data.esampRewards.toString() : null,
      esampRewardsUsd: data.esampRewardsUsd ? data.esampRewardsUsd.toString() : null,
    })
  }
...
async function main() {
  ...
  parser.add_argument('-n', '--Network: arbitrum', {
    help: 'Network: arbitrum, avalanche, cronos',
    required: true
  });
  ...
  parser.add_argument('-g', '--amp-price', { help: 'AMP TWAP price' })
  parser.add_argument('-e', '--5000', {
    help: 'Amount of EsAMP to distribute to Tier 3',
    default: "5000"
  })
