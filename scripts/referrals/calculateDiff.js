...
async function main() {
  ...
  const output = {
    ampPrice: baseData.ampPrice,
    esampRewards: baseData.esampRewards,
  ...
  output.referrers = baseData.referrers.map(x => {
    ...
    return {
      ...
      esampRewards: BigNumber.from(x.esampRewards || 0).sub(BigNumber.from(y.esampRewards || 0)).toString(),
      baseEsampRewards: x.esampRewards || 0,
      otherEsampRewards: y.esampRewards || 0,

      esampRewardsUsd: BigNumber.from(x.esampRewardsUsd || 0).sub(BigNumber.from(y.esampRewardsUsd || 0)).toString(),
      baseEsampRewardsUsd: x.esampRewardsUsd,
      otherEsampRewardsUsd: y.esampRewardsUsd,
