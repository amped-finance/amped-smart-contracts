...
async function main() {
  const stakedAmpTracker = await contractAt("RewardTracker", "0x2bD10f8E93B3669b6d42E74eEedC65dd1B0a1342")
  const bonusAmpTracker = await contractAt("RewardTracker", "0x908C4D94D34924765f1eDc22A1DD098397c59dD4")
  const feeAmpTracker = await contractAt("RewardTracker", "0x4d268a7d4C16ceB5a606c173Bd974984343fea13")
  ...
  await printRewardTracker(stakedAmpTracker, "stakedAmpTracker")
  await printRewardTracker(bonusAmpTracker, "bonusAmpTracker")
  await printRewardTracker(feeAmpTracker, "feeAmpTracker")

  await printRewardTracker(stakedAlpTracker, "stakedAlpTracker")
  await printRewardTracker(feeAlpTracker, "feeAlpTracker")

  const alp = await contractAt("MintableBaseToken", "0x01234181085565ed162a948b6a5e88758CD7c7b8")
  const usdg = await contractAt("USDG", "0xc0253c3cC6aa5Ab407b5795a04c28fB063273894")
  // const amp = await contractAt("MintableBaseToken", "0x62edc0692BD897D2295872a9FFCac5425011c661")
  // const esAmp = await contractAt("MintableBaseToken", "0xFf1489227BbAAC61a9209A08929E4c2a526DdD17")
  const bnAmp = await contractAt("MintableBaseToken", "0x8087a341D32D445d9aC8aCc9c14F5781E04A26d2")

  await printToken(alp, "alp")
  await printUsdg(usdg, "usdg")
  // await printToken(amp, "amp")
  // await printToken(esAmp, "esAmp")
  await printToken(bnAmp, "bnAmp")
  ...
  // await signalGov(prevGov, alp, nextGov, "alp")
  // await signalGov(prevGov, amp, nextGov, "amp")
  // await signalGov(prevGov, esAmp, nextGov, "esAmp")
  // await signalGov(prevGov, bnAmp, nextGov, "bnAmp")

  await updateToken(amp, "amp")
  await updateToken(esAmp, "esAmp")
  await updateToken(bnAmp, "bnAmp")

  await updateHandler(stakedAmpTracker, "stakedAmpTracker")
  await updateHandler(bonusAmpTracker, "bonusAmpTracker")
  await updateHandler(feeAmpTracker, "feeAmpTracker")
  await updateHandler(stakedAlpTracker, "stakedAlpTracker")
  await updateHandler(feeAlpTracker, "feeAlpTracker")

  await updateRewardTrackerGov(stakedAmpTracker, "stakedAmpTracker")

  await updateRewardTrackerGov(bonusAmpTracker, "bonusAmpTracker")
  await updateRewardTrackerGov(feeAmpTracker, "feeAmpTracker")
  await updateRewardTrackerGov(stakedAlpTracker, "stakedAlpTracker")
  await updateRewardTrackerGov(feeAlpTracker, "feeAlpTracker")

  await updateGov(alp, "alp")
  await updateGov(usdg, "usdg")
  // await updateGov(amp, "amp")
  // await updateGov(esAmp, "esAmp")
  await updateGov(bnAmp, "bnAmp")
  ...
  const alpManager = await contractAt("AlpManager", "0xe1ae4d4b06A5Fe1fc288f6B4CD72f9F8323B107F")
