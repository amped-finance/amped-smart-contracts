...
const stakeAmpList = require("../../data/stakeAmpList.json")
...
async function main() {
  ...
  for (let i = 0; i < stakeAmpList.length; i++) {
    const { address, balance } = stakeAmpList[i]

    const stakedAmount = await stakedAmpTracker.stakedAmounts(address)
    ...
    const pendingRewards = await stakedAmpTracker.claimable(address)
    const pendingBonus = await bonusAmpTracker.claimable(address)
