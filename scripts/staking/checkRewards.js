...
async function main() {
  ...
  const stakedAmpDistributor = await getDistributor(stakedAmpTracker)

  const bonusAmpTracker = await contractAt("RewardTracker", "0x4d268a7d4C16ceB5a606c173Bd974984343fea13")
  const bonusAmpDistributor = await getDistributor(bonusAmpTracker)

  const feeAmpTracker = await contractAt("RewardTracker", "0xd2D1162512F927a7e282Ef43a362659E4F2a728F")
  const feeAmpDistributor = await getDistributor(feeAmpTracker)

  const stakedAlpTracker = await contractAt("RewardTracker", "0x1aDDD80E6039594eE970E5872D247bf0414C8903")
  const stakedAlpDistributor = await getDistributor(stakedAlpTracker)

  const feeAlpTracker = await contractAt("RewardTracker", "0x4e971a87900b931fF39d1Aad67697F49835400b6")
  const feeAlpDistributor = await getDistributor(feeAlpTracker)

  await printDistributorBalance(esAmp, stakedAmpDistributor, "esAmp in stakedAmpDistributor:")
  await printDistributorBalance(bnAmp, bonusAmpDistributor, "bnAmp in bonusAmpDistributor:")
  await printDistributorBalance(weth, feeAmpDistributor, "weth in feeAmpDistributor:")
  await printDistributorBalance(esAmp, stakedAlpDistributor, "esAmp in stakedAlpDistributor:")
  await printDistributorBalance(weth, feeAlpDistributor, "esAmp in feeAlpDistributor:")
}
