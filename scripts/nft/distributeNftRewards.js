...
const distribute = async () => {
  ...
  const stakedAmpBalances = await processFile(inputDir + "staked-amp-balances.csv")
  const vestedBalances = await processFile(inputDir + "vested-balances.csv")

  const holders = {}

  for (let i = 0; i < stakedAmpBalances.length; i++) {
    const tokenHolder = stakedAmpBalances[i]
  ...
  if (shouldSendTokens) {
    await sendTxn(esAmp.approve(batchSender.address, expandDecimals(totalEsAmp, 18)), "esAmp.approve")
  }
  ...
  for (let i = 0; i < balanceList.length; i++) {
    ...
    const esAmpValue = (totalEsAmp - 1) * balance / totalBalance
    const esAmpAmount = ethers.utils.parseUnits(esAmpValue.toFixed(4), 18)

    accounts.push(account)
    amounts.push(esAmpAmount)
    totalEsAmpAmount = totalEsAmpAmount.add(esAmpAmount)

    console.log(`${i+1},${account},${esAmpValue},${esAmpAmount.toString()}`)

    if (accounts.length === batchSize && shouldSendTokens) {
      console.log("sending batch", i, accounts.length, amounts.length)
      await sendTxn(batchSender.send(esAmp.address,  accounts, amounts), "batchSender.send")
  ...
  console.log("totalEsAmpAmount", totalEsAmpAmount.toString())
}
