...
async function getAmpPrice(ethPrice) {
...
async function getArbValues() {
  ...
  const stakedAmp = await contractAt("Token", "0xd2D1162512F927a7e282Ef43a362659E4F2a728F", signer)
  const stakedAmpSupply = await stakedAmp.totalSupply()
  const { totalTransferAmount: keeperCosts } = await getArbKeeperValues()
  const alpManager = await contractAt("AlpManager", "0x321F653eED006AD1C29D174e17d96351BDe22649", signer)
  const alpAum = await alpManager.getAum(true)

  return { vault, reader, tokens, tokenInfo, nativeTokenPrice, feesUsd, stakedAmp, stakedAmpSupply, keeperCosts, alpManager, alpAum }
}
...
async function getAvaxValues() {
  ...
  const nativeTokenPrice = tokenInfo[tokens.nativeToken.address].maxPrice
  const feesUsd = await getFeesUsd(vault, reader, tokenInfo)
  const stakedAmp = await contractAt("Token", "0x4d268a7d4C16ceB5a606c173Bd974984343fea13", signer)
  const stakedAmpSupply = await stakedAmp.totalSupply()
  const { totalTransferAmount: keeperCosts } = await getAvaxKeeperValues()
  const alpManager = await contractAt("AlpManager", "0xe1ae4d4b06A5Fe1fc288f6B4CD72f9F8323B107F", signer)
  const alpAum = await alpManager.getAum(true)
...
async function main() {
  ...
  const ampPrice = await getAmpPrice(ethPrice)

  const data = [
    ["ETH Price", formatAmount(ethPrice, 30, 2)],
    ["AVAX Price", formatAmount(avaxPrice, 30, 2)],
    ["AMP Price", formatAmount(ampPrice, 30, 2)],
    ["ARB Fees", formatAmount(values.arbitrum.feesUsd, 30, 2)],
    ["AVAX Fees", formatAmount(values.avax.feesUsd, 30, 2)],
    ["ARB sbfAMP", formatAmount(values.arbitrum.stakedAmpSupply, 18, 2)],
    ["AVAX sbfAMP", formatAmount(values.avax.stakedAmpSupply, 18, 2)],
    ...
    ["ALP AUM (ARB)", formatAmount(values.arbitrum.alpAum, 30, 2)],
    ["ALP AUM (AVAX)", formatAmount(values.avax.alpAum, 30, 2)],
  ]
