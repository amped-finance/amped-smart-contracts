...
const holderList = require("../../data/holders/ampHolders.json")
...
async function main() {
  ...
  for (let i = 3490; i < holderList.length; i++) {
    ...
    const bnAmpBalance = await feeAmpTracker.depositBalances(account, bnAmp.address)
    const pendingRewards = await bonusAmpTracker.claimable(account)
    const totalRewards = bnAmpBalance.add(pendingRewards)
    console.log(`${i+1},${account},${ethers.utils.formatUnits(bnAmpBalance, 18)},${ethers.utils.formatUnits(pendingRewards, 18)},${ethers.utils.formatUnits(totalRewards)}`)
