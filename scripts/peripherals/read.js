...
async function readAlpManager(alpManager) {
  console.log("alpManager.cooldownDuration", (await alpManager.cooldownDuration()).toString())
}
...
async function main() {
  ...
  // await readAlpManager(alpManager)

  // await getPool(weth.address, usdc.address, 500)
  // await getPool(weth.address, amp.address, 10000)
  ...
  startTime = Date.now()
  await alpManager.getAums()
  console.log("alpManager.getAums", Date.now() - startTime)
}
