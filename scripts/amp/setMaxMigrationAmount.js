...
async function main() {
  ...
  const ampMigrator = await contractAt("AmpMigrator", "0x0472F402EA8E301D7595545884Ad4C420E9865d6")
  const gmt = {
    name: "GMT",
    contract: await contractAt("Token", "0x99e92123eB77Bc8f999316f622e5222498438784")
  }
  const xgmt = {
    name: "XGMT",
  ...
  if (shouldApprove) {
    for (let i = 0; i < tokens.length; i++) {
      ...
      const migratedAmount = await ampMigrator.migratedAmounts(account, token.contract.address)
      ...
      await sendTxn(ampMigrator.setMaxMigrationAmount(account, token.contract.address, totalAmount), message)
    }
  }
}
