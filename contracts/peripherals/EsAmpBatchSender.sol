...
contract EsAmpBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esAmp;

    constructor(address _esAmp) {
        admin = msg.sender;
        esAmp = _esAmp;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsAmpBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esAmp).transferFrom(msg.sender, _accounts[i], _amounts[i]);
