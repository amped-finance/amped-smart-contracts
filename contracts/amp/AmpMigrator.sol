...
import "./interfaces/IAmpIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/IAmpMigrator.sol";

contract AmpMigrator is ReentrancyGuard, IAmpMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "AmpMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "AmpMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "AmpMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "AmpMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "AmpMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "AmpMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "AmpMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "AmpMigrator: invalid _lpTokenBs.length");
    ...
    function migrate(
        ...
        require(isMigrationActive, "AmpMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "AmpMigrator: token not whitelisted");
        require(_tokenAmount > 0, "AmpMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "AmpMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(ampPrice);
        require(mintAmount > 0, "AmpMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "AmpMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "AmpMigrator: invalid tokenA");
            require(tokenB != address(0), "AmpMigrator: invalid tokenB");
        ...
        IAmpIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "AmpMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "AmpMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "AmpMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "AmpMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("AmpMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "AmpMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "AmpMigrator: invalid _action");
